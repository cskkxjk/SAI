...
- **语音短语替换（测试中）**：「热词与替换」新增「语音短语」页——把常用的短句录成模板并标注文字（0.5~4 秒，最多 50 条，支持试听、删除与合并重复样本）。识别时在音频层匹配，命中后整段替换为标注文字，优先级高于 `hot.txt` 热词与 `hot-rule` 规则。默认关闭，可在「选项」页开启；录音键为听写主快捷键（macOS 默认右 Option，Windows 默认 CapsLock）。
- **语音短语修复与加固**（代码审计）：命中匹配在发送最终音频前完成，首次识别不再丢失替换；匹配结果在所有结束路径（含空白//sil 与错误）都会清理；繁体转换移到替换之后，繁体用户不再失效；token 与输出文本不一致时按 difflib 映射回文本；长按录音键期间不再拦截其它按键（Windows 与 macOS 两条路径）；录制标记增加 120 秒超时清理、损坏容错与原子写入，强杀后不再卡住麦克风；面板窗口失焦立即交还录音键；修复麦克风预热旧流误绑定；macOS CapsLock 无法按住说话时提示更换按键。
- **macOS 支持范围**：最低系统要求为 **13.3+**（Ventura 及更新）；Apple Silicon 的 onnxruntime 固定 1.23.2（与 Intel 相同），修复 Ventura 上无法启动的问题。打包不再 strip 主程序（此前会删掉 PyInstaller 内嵌归档导致应用无法启动），只 strip 动态库，构建与 CI 增加归档校验。
- **清理**：移除语音短语匹配器中未使用的重复 DTW 实现与死代码，索引文件改为原子写入，清理未使用的导入。
...
- **macOS 支持 Intel 芯片**：新增 Intel（x64）架构支持（macOS 13.3+），与 Apple Silicon 版本同 tag 发布；同时提供 `.dmg` 拖拽安装镜像（arm64 / x64 均有）。
...
- **支持范围更正**：最低系统要求由 macOS 12 更正为 **13.3+**（Ventura 及更新）。Apple Silicon 的 onnxruntime 固定到 1.23.2（与 Intel 相同，要求 13.0），此前使用的最新版要求 macOS 14；GGUF 引擎运行库目标为 13.3。`Info.plist` 与文档同步更正。
- **修复 macOS 打包**：打包时不再 strip 主程序（`strip` 会删除 PyInstaller 追加在可执行文件尾部的内嵌归档，导致应用无法启动），仅 strip 动态库；构建与 CI 增加归档校验，防止回归。
...
- **检查更新更耐限流**：GitHub API 对未认证请求限制每小时 60 次。被限流（403）或网络异常时自动改用 `releases.atom` 获取版本信息（不占 API 配额，代价是没有 SHA256、下载时不校验摘要）；两个来源都失败才提示，并且失败后 15 分钟即可重试，成功后才进入 24 小时间隔。
- **录音文件管理**：「录音」卡片新增「保存录音与日记」开关、「打开录音文件夹」按钮与「自动清理录音」设置（默认保留 3 天，填 `0` 表示永久保留）。客户端启动时自动删除超过保留期的录音文件，只删音频，日记文字保留；此前录音只能一直堆积。
- **数据目录可改**：配置、热词与角色、日志、录音与日记默认仍在 `%LOCALAPPDATA%\SAI`，但安装时可在「数据存放目录」页选择位置，装完后也能在「录音」卡片里点「更改...」换盘——会询问是否把已有数据整体迁移过去（同目录合并、冲突文件保留原样），重启后生效；位置记录在安装目录的 `data-dir.txt` 里，卸载时只删除该记录，个人数据不会被卸载程序删掉。
- **录音卡片排版修复**：修复「保存录音与日记」开关覆盖「录音设备」说明文字的重叠问题。
