...
- **快捷键支持区分左右修饰键**：左/右 Ctrl、Alt、Shift、Win 现在是各自独立的按键，可分别绑定互不冲突（此前右 Ctrl 会被当成左 Ctrl）；`ctrl` / `alt` / `shift` / `win` 这类不带左右的写法仍然匹配任意一侧。快捷键标签会显示成「左 Ctrl」「右 Alt」等。配置文件里也支持 `left_ctrl`、`right_shift`、`lwin` 等写法。
- **鼠标中键现在可以绑定为快捷键**：此前录制时能选到鼠标中键，但触发逻辑只监听侧键 1/2，中键实际不会生效。
