...
[`merge_by_text`](../util/server/text_merge.py) 是 SAI 中用于解决**语音切片重叠去重**的核心算法，采用**模糊文本匹配**策略，具有高鲁棒性和抗噪音能力。
...
`merge_by_text` 通过窗口搜索、分层匹配、智能去重的设计，有效解决了语音切片重叠问题。在实际使用中，能够处理大部分常见的识别异常，是 SAI 实现流畅语音识别体验的关键技术之一。
